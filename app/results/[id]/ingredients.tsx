...
export default function IngredientsScreen() {
  ...
  const [enrichedProduct, setEnrichedProduct] = React.useState<ProductResult | null>(null);
  ...
  React.useEffect(() => {
    ...
    const q = [result.product.product_name, result.product.brands].filter(Boolean).join(" ").trim();
    if (!q) return;
  ...
  const { raw: ingredientsRaw, display: ingredients } = getDisplayIngredientsFromProduct(productToUse ?? result.product);

  return (
    <View className="flex-1" style={{ backgroundColor: isDark ? "#000" : THEME.bgLight }}>
      <ResultHeader display={result} title="Ingredients" onChatOpen={() => setChatOpen(true)} />
        ...
        {ingredients.length > 0 ? (
          ...
          >
            <View className="flex-row items-center gap-2 mb-1">
              ...
              <Text className="text-base font-semibold" style={textWhite}>Ingredients</Text>
            </View>
          ...
          >
            <List size={48} color={isDark ? "#525252" : "#94a3b8"} strokeWidth={1.5} />
            {fetchingEnriched ? (
              <Text className="mt-4 text-center text-muted-foreground" style={textMuted}>Looking up ingredients…</Text>
            ) : (
              <Text className="mt-4 text-center text-muted-foreground" style={textMuted}>
                No ingredients data available. Scan the barcode or add them on the label screen for full details.
              </Text>
            )}
          </View>
        )}
