...
export default function HealthScreen() {
  ...
  return (
    <View className="flex-1" style={{ backgroundColor: isDark ? "#000" : THEME.bgLight }}>
      <ResultHeader display={result} title="Health insights" onChatOpen={() => setChatOpen(true)} />
      <ScrollView className="flex-1" contentContainerStyle={{ padding: 16, paddingBottom: insets.bottom + 24, paddingTop: 8 }} showsVerticalScrollIndicator={false}>
        {healthRisks.length > 0 ? (
          ...
          >
            <Text className="text-base font-semibold" style={textWhite}>Personalized health insights</Text>
                ...
                return (
                  <View
                    ...
                    style={{
                      ...
                      backgroundColor: isDark ? cfg.bg.replace("0.1", "0.12") : cfg.bg,
                    }}
                  >
                    <Icon size={22} color={cfg.border} strokeWidth={2} />
                    <View className="flex-1">
                      <Text className="font-semibold" style={textWhite}>{r.category}</Text>
                      <Text className="mt-1 text-sm leading-5" style={textWhite}>{r.message}</Text>
                    </View>
                  </View>
                );
              })}
          ...
          >
            <View className="flex-row items-center gap-2">
              ...
              <Text className="text-base font-semibold" style={{ color: isDark ? "#86efac" : THEME.primary }}>
                No concerns from your profile
              </Text>
            </View>
            <Text className="text-sm leading-5" style={textMuted}>
              This product has no known allergens, conditions, or diet conflicts based on your Health Profile. Add allergies, conditions, or goals in Profile to get personalized insights.
            </Text>
          </View>
        )}
      </ScrollView>
